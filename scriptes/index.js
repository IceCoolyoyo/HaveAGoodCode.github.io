...
(function () {
    ...
    async function click(init) {
        ...
        messages.forEach((chat, index) => {
            if (chat.type === DramaType.Ball && chat.originalMessage.includes(Setting.drama_time)) {
    ...
    async function processMessage(obj) {
        ...
        switch (message.type) {
            ...
            case DramaType.Image:
                document.getElementById(Setting.lessonMediaID).appendChild(message.obj);
                break;
    ...
    const init = (function () {
        ...
        function eventHook() {
            document.getElementById(Setting.ballFrameID).addEventListener('click', async () => await click(false));
        }
